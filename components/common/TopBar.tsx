...
const TopBar = ({ showSettings, showAddModal }:TopbarProps) => {
   ...
   const isDomainsPage = pathname === '/domains';
   ...
   return (
       ...
       ${isDomainsPage ? 'max-w-5xl lg:justify-between' : 'max-w-7xl lg:justify-end'}  bg-white lg:bg-transparent`}>
         ...
         {!isDomainsPage && pathname !== '/research' && (
            <Link legacyBehavior href={'/domains'} passHref={true}>
         ...
         <div className="topbar__right">
            ...
            border border-gray-200 lg:mt-2 lg:relative lg:block lg:border-0 lg:bg-transparent ${showMobileMenu ? 'block' : 'hidden'}`}>
               <li className={`block lg:inline-block lg:ml-5 ${pathname === '/domains' ? ' text-blue-700' : ''}`}>
                  <Link legacyBehavior href={'/domains'} passHref={true}>
                     <a className='block px-3 py-2 cursor-pointer'>
                        <Icon type="domains" color={pathname === '/domains' ? '#1d4ed8' : '#888'} size={14} /> Domains
                     </a>
                  </Link>
               </li>
               <li className={`block lg:inline-block lg:ml-5 ${pathname === '/research' ? ' text-blue-700' : ''}`}>
                  <Link legacyBehavior href={'/research'} passHref={true}>
                     <a className='block px-3 py-2 cursor-pointer'>
                        <Icon type="research" color={pathname === '/research' ? '#1d4ed8' : '#888'} size={14} /> Research
                     </a>
                  </Link>
               </li>
               ...
               <li className='block lg:inline-block lg:ml-5'>
                  <a className='block px-3 py-2 cursor-pointer' href='https://docs.SerpUpdate.com/' target="_blank" rel='noreferrer'>
