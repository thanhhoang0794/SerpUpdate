...
const parseKeywords = (allKeywords: Keyword[]) : KeywordType[] => {
   const parsedItems = allKeywords.map((keyword:Keyword) => ({
         ...keyword,
         history: keyword.history as KeywordHistory,
         tags: keyword.tags,
         lastUpdateError: typeof keyword.last_update_error === 'string' && keyword.last_update_error !== 'false' && (keyword.last_update_error as string).includes('{') ? JSON.parse(keyword.last_update_error as string) : false,
         domain: keyword.domain_id,
         lastResult: keyword.last_result as KeywordLastResult[],
      }));
